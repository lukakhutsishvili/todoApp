...
function App() {
  ...
  const handlesubmit = (e) => {
    ...
    if (value.trim() !== "") {
      setnewitem((previous) => [
        ...previous,
        { id: crypto.randomUUID(), text: value, Checked: false },
      ]);
      setnewvalue("");
    }
  };
  ...
  const handleClearCompleted = () => {
    const remainingItems = newitem.filter((item) => !item.Checked);
    setnewitem(remainingItems);
  };

  const count = () => newitem.filter((item) => !item.Checked).length;
  ...
  const filteredItems = () => {
    switch (filter) {
      case "active":
        return newitem.filter((item) => !item.Checked);
      case "completed":
        return newitem.filter((item) => item.Checked);
      default:
        return newitem;
      ...
      <>
        <main style={!light ? { background: "#171823" } : {}}>
          <img
            className="mobile"
            src={`${
              light ? "/bg-mobile-dark.jpg" : "/bg-mobile-dark.jpg"
            }`}
